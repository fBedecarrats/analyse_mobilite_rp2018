...
amgts <- paste0(loc_geovelo, amgts_file) %>%
  read_sf() %>%
  mutate(azimut_amgt = line_bearing(.)) %>%  # On calcule les gisements avant projection
  st_transform(crs = my_crs) # on la passe en CC47
```
...
map2 <- tm_shape(amgts_focus) + 
  tm_lines(col = "green")+ 
tm_shape(freqs_focus) + 
  tm_lines(col = "orange", lwd = "frequency", scale = 5) + 
tm_shape(focus) + 
  tm_borders() + 
tm_legend() 
map2
