...
La pratique du vélo constitue un enjeu de politique publique important. Elle répond à une demande de mobilité des populations et son essor contribue à la réduction des émissions de gaz à effet de serre et à l'activité physique, en évitant de contribuer à certaines nuisances associées aux modes de transport motorisés (bruit, qualité de l'air, emprise sur l'espace public, congestion). Les collectivités locales disposent d'une palette moyens d'action pour favoriser la pratique du vélo, en particulier l'aménagement cyclable, c'est-à-dire la création, l'amélioration et la maintenance de dispositifs de voirie organisant la circulation et le stationnement des vélos. 
...
On crée une routine permettant de focaliser sur une zone spécifique afin de visualiser les aménagements cyclables (en vert) et les fréquentations d'itinéraires relevées par Géovélo (en orange) à une échelle où peut visuellement en vérifier la cohérence.  On observe sur la figure 1.1 qu'il y a un décalage entre la localisation des aménagements et celles des traces Geovélo. La figure 1.2 reproduit les mêmes éléments, mais en représentant le tracé des itinéraires Géovélo avec une épaisseur proportionnelle au nombre de passages relevés.

Pour attribuer aux aménagements les fréquentations itinéraires relevées par Géovélo, on constitue des zones tampon autour de des aménagements et on considère que les itinéraires Géovélo qui traversent ces zones tampons transitent par l'aménagement. La figure 1.3 représente un pemier essai avec de constitution de tampons "simples" avec une distance de 10 mètres.

On constate que ces tampons "simples" se superposent. Cela entraîne dans les zones de recouvrement une assignation d'un itinéraire Géovélo à plusieurs zones tampon et donc à plusieurs aménagements. Cette situation est problématiques, notamment aux carrefours où une même trace serait comptabilisées plusieurs fois. 

On définit donc une méthode permettant de générer des tampons contigus, c'est-à-dire qui ne se superposent pas. La méthode s'appuie sur la constitution de diagrammes de Voronoï [référence]. Cette procédure est exigentes en termes de calcul, car les diagrammes de Voronoï prennent comme référence des points et pas des lignes. Ici on doit décomposer chaque ligne en une série de points rapprochés, pour ensuite fusionner les polygonnes du diagramme de Voronoï appartenant à une même ligne (figure 1.4). 

### Fig.1 : Illustration des étapes de traitement pour l'affectation des traces Géovélo aux aménagements
...
tm_shape(focus) + 
  tm_borders() + 
tm_legend() +
  tm_layout(main.title = "Fig. 1.1 Tracés simples en contexte",
            main.title.size = main_title_size )
# Map2 with lines proportional to frequency
map2 <- tm_shape(rename(freqs_focus, `Passages` = frequency)) + 
  tm_lines(col = "orange", lwd = "Passages", scale = 5) + 
  tm_shape(amgts_focus) + 
  tm_lines(col = "green")+ 
  tm_shape(focus) + 
  tm_borders() + 
  tm_layout(main.title = "Fig. 1.2 Tracés proportionels aux passages",
...
map3 <- tm_shape(amgts_focus_buff10) + 
  ...
  tm_shape(focus) + 
  tm_borders() +
  tm_layout(main.title = "Fig. 1.3 Tampons simples",
            main.title.size = main_title_size)
...
map4 <- tm_shape(amgts_focus_buff10b) + 
  ...
  tm_add_legend(type = "fill", col = "green", alpha = 0.1, 
                labels = "Contiguës") +
  tm_layout(main.title = "Fig. 1.4 Tampons contigus",
            main.title.size = main_title_size, legend.outside = TRUE)
...
# On généralise en une fonction pour la suite
prepare_freq <- function(frequencies, buffers, angle_max = 20) {
  frequenciesb <- frequencies %>%
    st_split(buffers) %>%
  ...
  frequencies_in <- frequenciesb %>%
    filter(angle_diff <= angle_max | angle_diff >= 180 - angle_max)
}

# # On vérifie que la fonction produit bien le même résultat
# freq_focus_in2 <- prepare_freq(frequencies = freqs_focus,
#                                buffers = amgts_focus_buff10b)
# identical(freq_focus_in, freq_focus_in2)
...
map5 <- tm_shape(amgts_focus_buff10b) + 
  ...
  tm_shape(focus) + 
  tm_borders() +
  tm_layout(main.title = "Fig. 1.5 Sélection des tracés à affecter",
            main.title.size = main_title_size)
...
stats_freq_buff <- function(freq_l, inv_buff_s) {
  freq_inv_buff <- freq_l %>%
    ...
    mutate(gv_prod_len_freq = geovelo_paths_length * geovelo_frequency) %>%
    group_by(gid) %>% # Etape 3
    summarize(facility_sum_prod_len_freq = sum(gv_prod_len_freq, 
                                               na.rm = TRUE))  %>%
    st_drop_geometry()
}

summarise_freq <- function(frequencies, buffers, facilities) {
  
freq_per_buffers <- frequencies %>%
  stats_freq_buff(buffers)

facilities %>% 
  mutate(facility_length = as.numeric(st_length(.))) %>%
  left_join(freq_per_buffers, by = "gid") %>%
  mutate(facility_frequency = facility_sum_prod_len_freq / facility_length) %>%
  rename(Passages = facility_frequency)
}

freq_amgt_focus <- summarise_freq(frequencies = freq_focus_in,
                                  buffers = amgts_focus_buff10b,
                                  facilities = amgts_focus)
...
tm_shape(focus) + 
  tm_borders() + 
tm_legend() +
  tm_layout(main.title = "Fig. 1.6 Aménagements enrichis",
            main.title.size = main_title_size, legend.outside = TRUE) + 
  tm_add_legend(type = "line", labels = "Aménagements", col = "green") +
...
tmap_arrange(map1, map2, map3, map4, map5, map6, ncol = 2, 
             widths = c(0.43, 0.57))
```
...
1. On resegmente les itinéraires Géovélo à l'intersection des zones tampons pour que les segments se situent intégralement, soit à l'intérieur, soit à l'extérieur des zones tampons. Lors de cette opération, on reporte l'identifiant de l'aménagement correspondant à la zone tampon dans les attributs du segment de l'itinéraire Géovélo  ;
2. Pour chaque itinéraire se situant dans la zone tampon d'un aménagement, on calcule le gisement de l'itinéraire, c'est-à-dire son angle par rapport au Nord. On compare ce gisement à celui de l'aménagement correspondant. On associe à la zone tampon  tous les itinéraires présentant un angle inférieur à 20° par rapport à l'orientation de l'aménagement ;
3. Pour chaque segment d'itinéraire Géovélo associé à une zone tampon, on calcule le produit de la fréquence des passages relevés sur l'itinéraire Géovélo et de la longueur du segment ;
4. Pour chaque zone tampon on réalise la somme des produits calculés à l'étape 3 ; 
5. On reporte les sommes réalisées en 4. vers les aménagements associés ;
6. Pour chaque aménagement, on divise le produit calculé à l'étape 4 par la longueur de l'aménagement. 

## Analyses en fonction de l'aménagement

L'analyse par type d'aménagements sur décompose en deux étapes. Tout d'abord l'établissement de statiques de passages moyens par type d'aménagement pour identifier si la typologie des aménagements indique une différence. Ensuite, analyser la variabilité au sein de chaque catégorie d'aménagement afin d'identifier les aménagements présentant un faible niveau de protection qui sont les plus fréquentés, ou, au contraire, ceux offrant le plus haut niveau de protection mais qui sont peu fréquentés.
...
buffer_distances <- 10 # On simplifie, mais ça peut être c(5, 10, 15, 20)

for (i in 1:length(buffer_distances)) {
  output_filename <- paste0("amgt_buff_", 
                            buffer_distances[i], 
                            "m_contigus")
  input_path <- paste0(loc_geovelo, output_filename) 
  output_path <- paste0("temp/", output_filename)
  if (!file.exists(paste0(input_path, ".zip"))) {
    time_start <- Sys.time()
    # On applique donc à l'ensemble des aménagements
    buffered_no_overlap <- amgts %>%
      st_transform(my_crs) %>%
      buffer_no_overlap(group = c("gid", "azimut_amgt"),
                        dist = set_units(buffer_distances[i], m), # buffer distance
    ...
    # 5h30 pour le dernier batch à 10m.
    # A tester
    setwd("./temp")
    zip(zipfile = paste0(output_filename, ".zip"), 
        files = list.files(pattern = output_filename))
    setwd("../")
    file.copy(from = paste0("temp/", output_filename, ".zip"),
              to = paste0("data/Geovelo/", output_filename, ".zip"))
    # rm(buffered_no_overlap)
### Ne fonctionne pas :    
    # if (Sys.info()[['user']] == "rstudio") {
    #   library(aws.s3)
    #   bucket <- arrow::s3_bucket("minio.lab.sspcloud.fr/", 
    #                              region = "",
    #                              access_key =  Sys.getenv("AWS_ACCESS_KEY_ID"),
    #                              secret_key = Sys.getenv("AWS_SECRET_ACCESS_KEY"))
    #   object.copy(bucket = bucket,
    #               from_object = paste0(input_path, ".zip"))
    # }
...
for (i in 1:length(buffer_distances)) {
  unzip(paste0("data/Geovelo/", output_filename, ".zip"), exdir = "temp")
  temp <- st_read(paste0("temp/", output_filename, ".shp"),
                  quiet = TRUE)
  st_crs(temp) <- my_crs
  assign(paste0("amgt_buff_", buffer_distances[i], "m"), temp)
  rm(temp)
}

amgt_buff_10m_wgs <- amgt_buff_10m %>%
  st_transform(crs = "EPSG:4326")
freq_in <- prepare_freq(frequencies = freqs, 
                        buffers = amgt_buff_10m_wgs)

freq_amgt <- summarise_freq(frequencies = freq_in,
                                  buffers = amgt_buff_10m_wgs,
                                  facilities = amgts)
...
```{r eval, eval=FALSE, include=FALSE}
