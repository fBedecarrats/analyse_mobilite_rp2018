...
On crée une routine permettant de focaliser sur une zone spécifique afin de visualiser les aménagements cyclables (en vert) et les traces Géovélo (en orange) à une échelle où peut visuellement en vérifier la cohérence. 
...
Pour attribuer les traces Géovélo aux aménagements, on constitue des zones tampon autour de ces derniers et on considère que les traces qui traversent ces zones tampons transitent par l'aménagement.
...
On constate que ces tampons se superposent. Cela entraîne des difficultés d'assignation des traces qui se situeraient sur une aire de superposition entre deux zones tampon : à quel aménagements les assigner dans ces cas ? Par défaut, elles seraient comptabilisées deux fois. Cette situation serait particulièrement problématiques aux carrefours, où les traces seraient comptabilisées un grand nombre de fois. 
...
1. resegmentation des traces Géovélo pour que les segments se situent soit en dehors, soit dans les zones tampons ;
2. association à chaque trace Géovélo de l'identifiant de la zone tampon dans laquel elle se situe ;
3. Pour chaque identifiant de zone tampon, 
3. report des statistiques de passages réalisés en 2. vers les aménagements.
