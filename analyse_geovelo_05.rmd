...
3. report des statistiques de passages réalisés en 2. vers les tampons puis les aménagements associés ;
4. Pour chaque aménagement, diviser le produit calculé à l'étape 2. par la longueur de l'aménagement associé.


```{r}

freq_focus_in <- st_intersection(freq_focus, inv_buff_10m_focus)

freq_inv_buff_10m_focus <- freq_focus %>%
  st_intersection(inv_buff_10m_focus) %>%
  mutate(geovelo_paths_length = st_length(.),
...
freq_inv_focus <- inv_focus %>%
  ...
  mutate(facility_length = st_length(.),
         facility_frequency = paths_sum_frequency / facility_length)
